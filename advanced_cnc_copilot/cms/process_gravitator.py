...
PARADIGM: Process Scheduling via Gravitational Physics (Theory 1: Gravitational Scheduling)
Processes have Mass (Complexity) and Velocity (Priority).

v2.1 SPEC:
- Mass (Complexity): High geometric complexity creates stronger pull.
- Velocity (Priority): Tight deadlines increase kinetic energy/priority.
- Orbit: High-mass/velocity jobs gravitate to robust spindles (cores).
"""
...
class GravitationalProcess:
    """
    Process with gravitational properties grounded in Theory 1.
    """
    process_id: str
    complexity: float  # Theory: Mass
    deadline: float    # Theory: Inverse Velocity
    callback: Callable
    core_affinity: int = -1
    
    @property
    def mass(self) -> float:
        """Mass is derived from complexity."""
        return self.complexity * 10.0

    @property
    def velocity(self) -> float:
        """Velocity is derived from deadline urgency (Inverse)."""
        # Lower deadline values mean more urgent.
        return 1.0 / max(0.001, self.deadline)
...
class ProcessGravitator:
    ...
    def calculate_gravitational_force(self, 
                                     ...
                                     core_idx: int) -> float:
        """
        Theory 1: Calculate force with Velocity influence.
        F = G * (m_proc * v_proc * m_core) / r²
        """
        ...
        # Core "Resource Density" (Simulated Mass)
        # Robust spindles (cores 0, 1) have higher density to attract heavy jobs
        core_density = 2.0 if core_idx < 2 else 1.0
        
        # Gravitational force with Velocity (Kinetic) multiplier
        force = (self.GRAVITATIONAL_CONSTANT * 
                 (process.mass * process.velocity * core_density) / 
                 (distance ** 2))
