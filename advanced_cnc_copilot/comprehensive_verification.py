...
def check_container_status(service_name: Optional[str] = None) -> Dict[str, str]:
...
def test_background_jobs():
    ...
    try:
        ...
        success = response.status_code in [200, 201, 422]  # 422 means validation error but endpoint exists
        print(f"   Task queue endpoint: {'✅' if success else '❌'}")
        return success
    except Exception as e:
...
def test_file_upload():
    ...
    try:
        ...
        print(f"   Upload endpoint: {'✅' if success else '❌'}")
        return success
    except Exception as e:
...
def run_comprehensive_verification():
    ...
    for name, status in containers.items():
        status_icon = "[UP]" if "Up" in status else "[DOWN]"
    ...
    if passed == total:
        ...
        print(f"\n⚠️  {failed_count} out of {total} components have issues")
