...
class SimulationAgent:
    ...
    def _run_thermal_displacement(self, query: Dict, voxel_grid: np.array) -> Dict:
        """
        Estimates thermal growth and temperature distribution.
        """
        rpm = query.get("rpm", 5000)
        feed = query.get("feed", 500)
        
        # Simple Spindle Thermal Model: Temp increases with RPM^1.5 and Feed^0.5
        # base_temp + scaling * (rpm^1.5 * feed^0.5)
        temp_c = 25 + (0.00001 * (rpm**1.5 * feed**0.5))
        
        return {
            "type": "THERMAL_STEADY_STATE",
            "max_temp_c": temp_c,
            "expansion_mm": (temp_c - 25) * 0.000012, # Steel expansion 12um/m/C
            "gradient": "uniform"
        }

    def predict_spindle_temp(self, rpm: int, feed: float) -> float:
        """Lightweight thermal predictor for real-time mutation biasing."""
        return 25 + (0.00001 * (rpm**1.5 * feed**0.5))
