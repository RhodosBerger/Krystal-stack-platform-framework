...
class Lexicon:
    ...
    # --- PHASE 14: ROBOTICS/COBOT PRIMITIVES ---
    
    def load_part(self, fixture_id: str = "A") -> List[str]:
        """Triggers Robotic Load Handshake."""
        return [
            f"(--- LEXICON: ROBOT_LOAD_PART [{fixture_id}] ---)",
            "M05 (STOP SPINDLE FOR ROBOT)",
            "G28 U0 W0 (ORIENT FOR ENTRY)",
            "M00 (WAIT FOR COBOT HANDSHAKE)",
            "(ROBOT_LOADING_COMPLETE)"
        ]

    def unload_part(self, fixture_id: str = "A") -> List[str]:
        """Triggers Robotic Unload Handshake."""
        return [
            f"(--- LEXICON: ROBOT_UNLOAD_PART [{fixture_id}] ---)",
            "M05 (STOP SPINDLE FOR ROBOT)",
            "G28 U0 W0 (ORIENT FOR ENTRY)",
            "M00 (WAIT FOR COBOT HANDSHAKE)",
            "(ROBOT_UNLOADING_COMPLETE)"
        ]

    def deburr_edge(self, length: float, complexity: float) -> List[str]:
        """Triggers Autonomous Robotic Deburring."""
        return [
            f"(--- LEXICON: DEBURR_EDGE [COMPLEXITY:{complexity:.2f}] ---)",
            "M01 (HANDOVER TO ROBOT)",
            f"(REQUEST_ROBOT_DEBURR_LEN_{length})"
        ]
    ...
    def get_lexicon_definitions(self) -> Dict[str, str]:
        ...
        return {
            ...
            "load_part": "Robotic Part Loading. Parameters: fixture_id",
            "unload_part": "Robotic Part Unloading. Parameters: fixture_id",
            "deburr_edge": "Robotic Deburring. Parameters: length, complexity",
