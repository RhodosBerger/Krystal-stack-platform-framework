...
# Import LLM backends
try:
    from .llm_backends import LLMRouter, LLMMessage, LLMProvider, create_llm_router
    HAS_LLM = True
except ImportError:
    HAS_LLM = False
    LLMRouter = None
...
class CoderAgent(LLMAgent):
    ...
    def __init__(self, agent_id: str = "coder", use_llm: bool = True):
        ...
        self.llm_router = create_llm_router() if self.use_llm else None
    ...
    def process(self, spec: Dict, context: Dict = None) -> GeneratedArtifact:
        ...
        if self.use_llm and self.llm_router:
            ...
            response = self.llm_router.complete(messages, max_tokens=1024)
            code = response.content
            self.metrics["tokens"] += response.tokens_used
        else:
            # Fallback mock
